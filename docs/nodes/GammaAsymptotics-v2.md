...
| Source | [Conclusions.lean](https://github.com/teorth/IEANTN/blob/main/IEANTN/Nodes/GammaAsymptotics/v2/Conclusions.lean#L61) |
...
> Imports `none`, and that is a real none: w is universally quantified with every condition a hypothesis, so a Lean proof is the whole justification. THIS IS v1 WITH A HEIGHT RESTRICTION, and the restriction is the point. v1's recorded route -- iterate digamma_apply_add_one from a compact box in Re w in [1,2] -- DOES NOT PROVE v1. It pushes rightward and controls Re w -> infinity at bounded height; it says nothing about w = 1 + it as t -> infinity, which no compact box reaches. That was noticed only when someone went to carry the route out. On a strip the same route works. The recurrence psi(w+1) = psi(w) + 1/w moves w rightward until its real part is large, the accumulated sum of 1/(w+j) is compared with the harmonic series to produce log w, and the starting region -- Re w in [1,2] with \|Im w\| <= H -- is genuinely compact, so psi is bounded on it by continuity, meromorphic_digamma putting the poles at the non-positive integers. THE CONSTANT MAY DEPEND ON H AND MUST. A bound uniform in H is exactly v1.
