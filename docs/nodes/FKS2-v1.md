...
> Two conversion pipelines carrying admissible classical bounds along psi -> theta -> pi, together with the explicit numerical bounds the paper obtains by running the best available inputs through them. This node exports four of those numerical outputs: Corollary 14, a classical bound for E_theta; Corollary 22, the headline asymptotic \|pi(x) - Li(x)\| <= 9.2211 x sqrt(log x) exp(-0.84768363 sqrt(log x)) for x >= 2; Corollary 23 at the [0.826, 0.25, 1.00, 1.000] row of Table 6; and Corollary 26, \|pi(x) - Li(x)\| <= 0.4298 x / log x for x >= 2. The pipelines themselves live on FKS2.v2, the pipeline variant, and are imported here rather than restated.
...
- Each conclusion is proved in Lean here, but only CONDITIONALLY: the step from its imports to its statement is what Comparator checked. The imports themselves are not proved here. In particular FKS2Numerics.v1's five finite computations are asserted on the paper's authority and have not been re-run, and FKS.v1 and BKLNW.v1 are cited, so anything downstream inherits all of that.
- Two errata in the source paper, both still present in the published version. Lemma 10's `a = 0` bullet states that g decreases for all log(x) > -2b/c, where its own proof derives that the derivative is negative when u < -2b/c -- the inequality is reversed. And Corollary 11 states B >= 1 + C^2/(16R), where discharging it through Lemma 10's first bullet needs the strict B > 1 + C^2/(16R); at equality one falls into the second bullet, which gives decrease only above a threshold. Neither affects the corollaries stated here, which do not route through the equality case.
...
- **agent** (Claude Opus 5 (Anthropic)) — Transcribed the four corollaries from the paper and wrote the Lean solution proving them from the imported pipelines and numerical inputs, under the direction of the maintainer. Comparator accepted that solution; the receipts record the runs.
