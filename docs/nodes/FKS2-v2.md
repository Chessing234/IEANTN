...
> The two transformations underlying Fiori-Kadiri-Swidinsky's explicit prime number theorem bounds, stated for arbitrary admissible parameters rather than at the paper's named constants: Proposition 13 converting a classical bound on E_psi into one on E_theta, and Theorem 3 converting one on E_theta into one on E_pi. Both are conditional theorems importing nothing, so a Lean proof is their whole justification. FKS2.v1 states the same paper's corollaries, which are instances of these.
...
| | |
|---|---|
| Lean name | `FKS2.v2.proposition_13` |
